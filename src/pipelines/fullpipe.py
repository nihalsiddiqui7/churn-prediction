...
# Add project root (parent of `src`) to sys.path so `import src...` works
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..")))

# Local modules - Core pipeline components
from src.data_d.load_data import load_data                    # Data loading with error handling
from src.data_d.preprocess_data import preprocess_data            # Basic data cleaning
from src.features.build_features import feature_engineering   # Feature engineering (CRITICAL for model performance)
from src.utils.validate import validate_data   # Data quality validation
...
def main(args):
    ...
    # Use repository root (parent of `src`) as project root so paths are consistent
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    # On Windows, ensure a valid file URI (file:///C:/...) or use provided URI
    if args.mlflow_uri:
    ...
    with mlflow.start_run():
        ...
        df = preprocess_data(df, args.target)  # Basic cleaning (handle missing values, fix data types)

        # Save processed dataset for reproducibility and debugging
        processed_path = os.path.join(project_root, "data", "processed", "final_churn_processed.csv")
        ...
        df_enc = feature_engineering(df)  # one-hot encoding
...
python scripts/run_pipeline.py \
    --input data/raw/customer_churn_dataset \
    --target Churn
